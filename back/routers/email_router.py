...
@router.get("/{addressed_id}/inbox",status_code=200,  response_model=List[EmailSchema])
async def get_user_inbox(addressed_id: str):
    inbox = await controller.get_inbox(ObjectId(addressed_id))
    return inbox

@router.get("/{addressee_id}/sent",status_code=200, response_model=List[EmailSchema])
async def get_user_sent_email(addressee_id: str):
    sent_emails = await controller.get_sent_emails(ObjectId(addressee_id))
    return sent_emails
